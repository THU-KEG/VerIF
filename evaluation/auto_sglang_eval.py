...
task_info_dict = {
    'simple_evals': {
        ...
        'trace': [
            'python3 evaluate.py --save_dir {model_path} --sglang_url {sglang_url} --gpt4_url {vllm_url} --backbone sglang --checker vllm --tasks ifeval cello cfbench followbench arena_hard alignbench --proc_num 512 --model_name {model_name} --auto_extract_answer --max_new_tokens 15000 --system_message "{system_message}"'
        ],
...
def export_wandb(iter_dir, task_info, project, wandb_entity):
    *mgt_path, iter_name = iter_dir.split('/')
    ...
    wandb_kwargs = {
        ...
        'group': exp_name,
        'project': project,
        'job_type': 'eval-deepthink',
        'name': exp_name,
        'id': exp_name + '-' + task_info['eval_group'] + "-reeval",
        'resume': "allow"
    }
...
if __name__ == '__main__':
    ...
    parser.add_argument('--vllm_url', type=str, default="http://172.20.66.119:8000/v1")
    parser.add_argument('--model_name', type=str, default="Meta-Llama-3.1-70B-Instruct")
    parser.add_argument('--system_message', type=str, default="deepthink")
